...
exports.rankingService = {
    ...
    async dashboardRecrutador(recrutadorId) {
        const [total, abertas, encerradas, totalCandidatos, top5] = await Promise.all([
            prisma_1.prisma.job.count({ where: { recrutadorId } }),
            prisma_1.prisma.job.count({ where: { recrutadorId, status: "ABERTA" } }),
            prisma_1.prisma.job.count({ where: { recrutadorId, status: "ENCERRADA" } }),
            prisma_1.prisma.application.count({ where: { vaga: { recrutadorId } } }),
        ...
        const media = await prisma_1.prisma.application.aggregate({
            where: { vaga: { recrutadorId } },
            _avg: { notaFinal: true },
        });
        ...
        const [vagasUltimos30, vagasAnteriores30, candidatosUltimos30, candidatosAnteriores30, mediaUltimos30, mediaAnteriores30,] = await Promise.all([
        ...
        const tendencias = {
            ...
            media: calcDelta(Math.round(mediaUltimos30._avg.notaFinal ?? 0), Math.round(mediaAnteriores30._avg.notaFinal ?? 0)),
        };
        // Atividades recentes (últimas 8 inscrições)
        const atividadesRecentes = await prisma_1.prisma.application.findMany({
            where: { vaga: { recrutadorId } },
    ...
    async dashboardCandidato(candidatoId) {
        const [vagasAbertas, minhasInscricoes, gh, submissoes] = await Promise.all([
            prisma_1.prisma.job.count({ where: { status: "ABERTA" } }),
            prisma_1.prisma.application.findMany({
                where: { candidatoId },
                include: {
            ...
            prisma_1.prisma.githubAnalysis.findUnique({ where: { candidatoId } }),
        ...
        for (const s of submissoes) {
            ...
            if (!atual || s.nota > atual.nota)
                melhorPorDesafio.set(s.challengeId, s);
        }
        const desafiosResolvidos = Array.from(melhorPorDesafio.values()).map((s) => ({
            submissionId: s.id,
            challengeId: s.challengeId,
            tipo: s.challenge.tipo,
            enunciado: s.challenge.enunciado.length > 80
        ...
        const mediaDesafios = desafiosResolvidos.length
            ? Math.round(desafiosResolvidos.reduce((acc, d) => acc + d.nota, 0) /
        ...
        return {
            ...
            reprovadas,
            emAnalise,
            mediaNotas: Math.round(minhasInscricoes.reduce((acc, i) => acc + i.notaFinal, 0) /
